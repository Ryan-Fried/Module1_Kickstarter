...
### From the outcomes based on goals we can conclude that there doesn't seem to be a significantly consistent or defining correlation between goal value and percentage of success. It is notable, however, that the likelihood of failure tends to increase as the goal increases, and conversely, the likelihood of success tends to decrease as the goal increases. It's important to consider that because percentage is used as the Y axis, the line graph does not consider the count of outcomes within each range. In the lower ranges, where the counts are much higher, the graph appears more stable and linear. In the higher ranges, the graphed percentages are based on far fewer data points resulting a more scattered and random looking visual, where a pattern is more difficult to discern. Refer to the graph pictured above entitled "Outcomes Based on Goal" for a visualization of this analysis.

### Though this dataset is incredibly extensive, there are always inevitable limitations. The most obvious limitation I can think of is the simple factor of subjectivity. Given the nature of Louise's project as an artistic work, many qualitative factors will play a part in the success or failure of her project, regardless of whatever data based recommendations and objective analysis we can provide her. To some extent the marketability and quality of her play could be evaluated, but this data set is focused on quantitative factors of kickstarter campaigns.

### We could continue to analyse this dataset even further to get an even clearer understanding of trends. One chart we could add would address the limitations of the "Outcomes Based on Goals" chart. In addition to a chart based on percentage, we coul create a chart with the count of outcomes on the Y Axis, which would act as valuable companions to each other. Additionally, we could create a third chart in which would weight datapoints graphed by percentage in respect to the count of outcomes within each goal range. This would ideally create a more linear relationship between goal value and success rate.
